...
void measure(int n, int m, int k, float t, double ple, int threads, int seed, int plot) {
    ...
    auto weights = satgirgs::generateWeights(n, ple, weightSeed, threads > 1);
...
int main(int argc, char* argv[]) {
    ...
    for(int rep=0; rep<reps; ++rep) {
		...
		if(deg*4 < i) 
		    measure(i, m, deg, t, ple, threads, ++seed, 0);
        }
        ...
        for(int i = 2; i<= 1<<6; i <<= 1) {
            clog << i << endl;
            measure(n, m, i, t, ple, threads, ++seed, 1);
        }

        clog << "shrinking t" << endl;
        for(auto i : {t, 5.0, 4.0, 3.0, 2.0, 1.0, 0.5, 0.0}) {
            clog << i << endl;
            measure(n, m, k, i, ple, threads, ++seed, 2);
        }
    }
